...
from .queries import get_easee_account, update_easee_account

logger = structlog.get_logger()


def with_easee_client[**P, R](
    func: Callable[Concatenate[EaseeClient, P], Awaitable[R]],
) -> Callable[P, Awaitable[R]]:
    """
    A decorator that injects an Easee client to the decorated function. The
    ...
    async def inner(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        account = await get_easee_account(account_id=account_id)
        async with EaseeClient(access_token=account.access_token) as client:
            try:
...
async def refresh_access_token(client: EaseeClient, *, account_id: int) -> None:
    """
    Refresh the Easee access token for the given account.
    """
    ...
    async with db.connection() as con:
        ...
        async with con.transaction():
            account = await get_easee_account(account_id=account_id, for_update=True)
